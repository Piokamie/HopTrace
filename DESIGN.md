# HopPath — deterministic multi-hop retrieval with traces

> Formerly "HopTrace", renamed before first release (ADR 0013); ADRs
> 0001–0012 keep the historical names.

HopPath is a local, LLM-free retrieval engine exposed over MCP. The intended
flow: the user asks their assistant (Claude or any MCP client) a question,
the assistant calls HopPath, HopPath returns evidence chunks *with the path it
...
re-ingest bills). HopPath's index is deterministic instead — rebuild is
...
**Validation gate.** Before any HopPath number is reported, HopPath's own BM25
...
construction and floor-vs-HopPath comparisons are biased toward
HopPath. That makes it unusable as proof and useful as plumbing — an
...
bracket(corpus_id, n_questions?)               → {floor, hoppath_1hop, hoppath_2hop, oracle, multihop_fraction, miss_breakdown, caveat}
```
...
2. **Inflected languages break trivial aliasing.** Case, punctuation and
   ...
   *Kowalskiego*, *Kowalskim* and *Kowalskiemu* are one entity indexed as
   ...
   HopPath is accurate on English and lossy on inflected languages.
...
- **Document formats**: PDF, DOCX, HTML as first-class inputs. Each is a
  ...
  optional extra (`hoppath[documents]`), never in the core dependency
  set.
...
- Not a vector-DB replacement. Hybrid setups (HopPath + dense retrieval as an
