...
HopPath does that, without a model. At ingest it builds an inverted
...
```
$ hoppath retrieve "What can the manager of Alicja Rud see from the window?" --corpus office -k 4
...
| MuSiQue — HopPath 2hop + rerank (measured here) | 54.2 | 66.1 |
| 2Wiki — HippoRAG, best variant (paper) | 71.5 | 89.5 |
| 2Wiki — HopPath 2hop + rerank (measured here) | 73.8 | 85.9 |
...
`hoppath eval`: the fine-tuned reranker ships with the repository (int8
...
the eval harness needs `hoppath[eval]` (numpy), the learned reranker
`hoppath[rerank]` (ONNX Runtime + tokenizers — no torch, no GPU), the
optional NER pass `hoppath[ner]` (spaCy).

```bash
uv sync            # or: pip install -e .  (extras: pip install -e '.[eval,rerank]')
uv run hoppath ingest examples/office --corpus office
uv run hoppath retrieve "What equipment is in the room where the calibration team meets?" --corpus office
uv run hoppath bracket --corpus office
```

Corpora are single SQLite files under `$HOPPATH_DATA_DIR` (default
`./.hoppath/`), replaced atomically on re-ingest.
...
```bash
uv run hoppath ingest /path/to/docs --corpus mydocs
uv run hoppath bracket --corpus mydocs -n 200
uv run hoppath retrieve "a question your documents answer" --corpus mydocs
```
...
- `bm25-floor` vs `hoppath@1hop` / `@2hop` — recall and all-gold at k.
...
it cannot see extraction misses and is biased toward HopPath. It is a
...
uv run hoppath retrieve "your question" --corpus mydocs --rerank
```

With no `--rerank-model`, this uses the fine-tuned path-aware model that
ships with the repository — `models/hoppath-rerank-minilm-l6/` holds the
...
`$HOPPATH_DATA_DIR/models/`, sha256-pinned): `--rerank-precision fp32`.
...
`--rerank-model /path/to/model-dir`, and `HOPPATH_RERANK_MODEL` sets
...
- `unresolved mentions: X` with `seed_source: bm25_only` — a capitalized
  ...
  to the top BM25 hit. Run `hoppath explain <chunk_id> --corpus X` to see which
...
stemming. Full flags: `hoppath <subcommand> --help`.

### MCP server

`hoppath serve` speaks MCP over stdio and exposes four tools:
...
```json
{
  "mcpServers": {
    "hoppath": {
      ...
      "env": {
        "HOPPATH_DATA_DIR": "/abs/path/to/hoppath/.hoppath"
      }
    }
  }
}
...
Both paths must be absolute. `HOPPATH_DATA_DIR` is not optional in
practice: it defaults to `./.hoppath` relative to the working directory,
...
`"HOPPATH_RERANK_MODEL": "/abs/path/to/model-dir"` to point it at
another artifact.

#### Network mode

The default above is stdio: the client launches `hoppath serve` itself
...
```bash
uv run hoppath serve --http --port 8000
# hoppath MCP server on http://127.0.0.1:8000/mcp (no auth)
```
...
[skills/hoppath/SKILL.md](skills/hoppath/SKILL.md) tells an agent how to
...
- **A multi-word name that only ever starts sentences loses its first
  ...
  *Priya Nair* that appears exclusively as a sentence opener therefore
  ...
  `hoppath explain <chunk_id> --corpus X`.
...
```bash
uv sync --extra eval --extra rerank
uv run hoppath eval --dataset beir-hotpotqa --gate   # downloads + builds once (about an hour; hardware-dependent)
...
uv run hoppath eval --dataset hipporag-musique --hops 2 --selection rerank --rerank-precision fp32 --diagnostics
uv run hoppath eval --dataset hipporag-2wiki --hops 2 --selection rerank --rerank-precision fp32 --diagnostics
# the zero-shot control row
uv run hoppath eval --dataset hipporag-musique --hops 2 --selection rerank --rerank-model ms-marco-minilm-l6-v2
# development setting (smaller haystack, not externally comparable)
uv run hoppath eval --dataset musique --hops 2 --diagnostics
```

Datasets download to `$HOPPATH_DATA_DIR/datasets/` with recorded checksums
