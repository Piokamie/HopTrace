...
Seeding is two-source: query mentions looked up in the inverted index
(exact canonical, then alias table), plus BM25 top-N over analyzed query
terms. Seed scores are query relevance in [0, 1]; expansion propagates
them through bridge strengths; selection is the per-ring interleave by
default, with greedy submodular admission behind a switch (ADR 0008). A
query whose mentions all fail to resolve falls back to BM25-only seeding,
and the result says so — that event also feeds the alias-miss
diagnostics.
"""
...
class Retriever:
    ...
    def retrieve(
        ...
        if not resolved:
            notes.append("seed_source: bm25_only (no query mention resolved)")

        terms = analyze(query, self._analyzer)
        seed_depth = max(self._cfg.seed_bm25_top_n, cfg_k)
        ...
        pool, touched = self._expander.expand(seeds, hops=cfg_hops)
        seed_top = tuple(
            c.chunk_id for c in sorted(seeds, key=lambda c: (-c.score, c.chunk_id))[:cfg_k]
        )
        ...
        if self._cfg.selection == "submodular":
            ...
            admitted = [
                Selected(candidate=c, novelty=1.0, gain=c.score) for c in interleave(pool, cfg_k)
            ]
        ...
        query_entities = {entity for _, entity in resolved}
        ...
        return RetrievalResult(
            ...
            candidates_examined=bm25_candidates + touched,
            pool_size=len(pool),
            pool_hops={cid: c.hop for cid, c in pool.items()},
            seed_top=seed_top,
            query_mentions=tuple(m.surface for m in mentions),
